...
static bool is_whitespace(int32_t lookahead) {
...
static bool find_match_end(TSLexer* lexer, bool skip_any) {
...
static bool Scanner_scan(Scanner* scanner, TSLexer* lexer, const bool* valid_symbols) {
    ...
    if (found_end_of_line) {
        ...
        if (valid_symbols[DEDENT] && scanner->previous_indent_length > 0 && indent_length == 0 && first_comment_indent_length < (int32_t)scanner->previous_indent_length) {
            scanner->previous_indent_length = indent_length;
